...
결과가 이 파일이다. 최초 버전이 만들어내던 HTML은 예외 없이 아래 한 가지
모양이었다.
...
"템플릿으로 찍어냈다"는 신호로 읽힌다.

**바로 다음 버전이 저지른 실수**: (1)을 고치겠다고 모든 글 맨 앞에 <h2>핵심
요약</h2><table>...을 고정으로 붙였다. 표 하나가 생겨서 "벽글"은 벗어났지만,
24편 전부가 글자 하나 다르지 않게 똑같은 태그로 시작하는 훨씬 더 강한
"찍어낸 페이지" 신호를 새로 만든 것이다. 구조를 획일화해서 개별 증상 하나를
고치면 반드시 다른 획일성이 남는다 — 표·소제목 개수·서론 공식처럼 **파이썬
쪽에서 고정값으로 박아 넣는 모든 것**이 잠재적 템플릿 지문이라는 뜻이다.

지금 버전이 견지하는 것:

- 결론 소제목을 LLM이 주제에 맞게 짓는다 (고정 리터럴 제거)
- 핵심 요약 표를 강제하지 않는다 — 서론이 바로 시작한다. 이미지·표가 없다는
  경고(quality.py no_visual)는 차단이 아니라 경고일 뿐이다
- 서론의 여는 방식과 소제목 개수(3~6개)를 매번 LLM이 주제에 맞게 고르게
  한다 — 파이썬이 개수나 형식을 고정하지 않는다
...
다음에 또 증상 하나가 보인다고 코드에서 구조를 하나 더 고정하고 싶어지면,
이 문단을 먼저 다시 읽을 것.
"""
...
1. ⚠️ 가장 중요 — **모르는 수치를 지어내지 마십시오.**
   당신에게는 실제 자료가 주어지지 않았습니다. 따라서 아래는 **절대 쓰지
   마십시오.** 하나라도 지어내면 그 글은 폐기됩니다.
   - 금액 기준 ("연소득 4,500만원 이하", "자산 1억 원")
   - 연령·기간 요건 ("만 19~39세", "결혼 후 7년 이내", "30세 이하")
   - 비율·요율·공제율 ("중위소득 80%", "공제율 15%")
   - 법령 조항 번호, 통계 수치, 시행일
   - 기관 이름을 추측해서 쓰는 것 (예: '인사청' 같은 존재하지 않는 기관)

   대신 이렇게 쓰십시오:
   - ❌ "연소득 4,500만원 이하면 신청할 수 있습니다"
   - ✅ "소득 기준은 가구원 수와 공고마다 다릅니다. 청약홈 공고문의
        '소득기준' 표에서 본인 가구원 수에 해당하는 금액을 확인하십시오"

   즉 **답 자체가 아니라 '답을 어디서 어떻게 확인하는지'를 쓰십시오.**
   확인 경로, 준비할 서류, 판단할 때 헷갈리는 지점, 흔한 실수 —
   이런 것은 수치 없이도 충분히 구체적으로 쓸 수 있고, 독자에게 더
   쓸모 있습니다.

2. 일반 상식으로 널리 통용되는 수치(예: 물 하루 8잔, 성인 권장 수면 7~9시간)는
   써도 되지만, "일반적으로 권장됩니다" 같은 표현으로 단정을 피하십시오.

3. "중요합니다", "도움이 됩니다" 같은 빈 문장으로 분량을 채우지 마십시오.

4. 서론은 매번 같은 방식으로 열지 마십시오. 아래 중 그 주제에 가장 자연스러운
   방식을 고르되, 같은 문서 안에서도 글마다 다른 방식을 쓰십시오.
   - 흔한 오해나 잘못된 통념을 짚으며 시작
   - 구체적인 상황·장면을 묘사하며 시작 ("~할 때, ~한 경험이 있다면")
   - 독자가 실제로 검색했을 법한 질문을 그대로 던지며 시작
   - 의외의 사실이나 잘 안 알려진 지점을 먼저 제시하며 시작
   "이 문제가 왜 생기는지 설명하겠습니다" 류의 설명문으로 시작하지 마십시오.

5. 소제목은 3~6개 사이에서, **그 주제를 풀어내는 데 실제로 필요한 개수만큼만**
   쓰십시오. 매번 5개나 7개를 채우려고 억지로 쪼개거나 합치지 마십시오. 개수를
   고정 공식처럼 맞추는 것 자체가 이전에 반려된 글들의 특징이었습니다.

6. 결론의 소제목은 주제에 맞게 직접 지으십시오. "마무리", "결론" 같은
   일반적인 단어를 그대로 쓰지 마십시오.

7. 과장 광고나 근거 없는 의학적·재무적 단정을 피하고, 확실하지 않은 것은
   "경우에 따라 다르다"고 정직하게 쓰십시오.

8. 아래처럼 내용 없이 분량만 채우는 상투어를 쓰지 마십시오: "다양한", "여러
   가지", "이러한 이유로", "~것이 중요합니다", "~것이 좋습니다", "살펴보겠습니다",
   "정리해보겠습니다". 대신 실제로 무엇을 확인하고 무엇을 하면 되는지 구체적
   동사로 쓰십시오.

9. 섹션 중 최소 한 곳에는 뜬구름 잡는 설명 대신 손에 잡히는 구체적 사례,
   비교, 또는 실제로 헷갈리는 지점 하나를 붙이십시오. 모든 섹션이 같은
   추상도로 쓰이면 템플릿처럼 읽힙니다.

10. 산문(서론+각 섹션 본문+결론)의 합계가 1,600자를 넘도록 충분히 상세하게
    쓰십시오. 다만 **분량을 채우려고 수치를 지어내는 것은 최악입니다.**
    분량이 모자라면 확인 절차와 주의점을 더 풀어 쓰십시오.

11. 아래 JSON 스키마 그대로만 반환하십시오 (마크다운 코드블록이나 설명 문장 금지).
...
"sections"는 3~6개, 주제에 맞게 필요한 만큼만 채웁니다.
"list"가 꼭 필요한 섹션에만 채우십시오. 모든 섹션에 리스트를 넣으면 그 자체가
템플릿처럼 보입니다 — 리스트 없는 섹션이 있는 편이 자연스럽습니다.
...
def render_html(post: dict, related: list, notice: str = "") -> str:
    ...
    # 1) 서론. 예전 버전은 여기 앞에 '핵심 요약' <table>을 고정으로 넣었다.
    #    표 자체는 문제가 아니지만, 24편 전부가 글자 하나 안 틀리고
    #    <h2>핵심 요약</h2><table>...로 시작한 것이 오히려 "찍어낸 페이지"라는
    #    가장 강한 신호였다 — quality.py의 no_visual 은 경고일 뿐 차단이
    #    아니므로, 표 없이 다양한 서론으로 시작하는 쪽이 더 안전하다.
    ...
    # 3) 결론. 소제목을 고정 리터럴로 박지 않는다 — 24편의 마지막 h2 가 전부
    ...
    # 4) 내부 링크. 없으면 블록 자체를 생략한다 (빈 목록은 오히려 나쁘다).
    if related:
        li = "".join(
        ...
        try:
            post = _call_groq(
                ...
                extra_instruction=(
                    ...
                    f"확인 절차·준비 서류·흔한 실수·헷갈리는 지점을 더 풀어 써서 산문 "
                    f"합계가 {MIN_PROSE_CHARS}자를 넘도록 다시 작성하세요.\n"
                    "⚠️ 분량을 채우려고 금액·연령·비율 같은 수치를 지어내지 마십시오. "
                    "그렇게 채운 글은 폐기됩니다."
                ),
            )
        except Exception as e:
